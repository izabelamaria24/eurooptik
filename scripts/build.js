...
async function build() {
    try {
        console.log("Starting root domain build...");
        ...
        for (const locale of locales) {
            ...
            const [
                ...
                contentful.fetchTeamFromContentful({ locale }), contentful.fetchServicesFromContentful({ locale }),
                contentful.fetchTestimonialsFromContentful({ locale }), contentful.fetchArticlesFromContentful({ locale }),
                contentful.fetchPricingData({ locale }), contentful.fetchSpecializationsData({ locale }),
            ...
            await fs.copy(path.join(projectRoot, 'index.html'), path.join(langOutputDir, 'index.html'));
            const pagesDir = path.join(projectRoot, 'pages');
            if (fs.existsSync(pagesDir)) {
                await fs.copy(pagesDir, path.join(langOutputDir, 'pages'));
            }
        }
        ...
        const redirectHTML = `<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0; url=/${defaultLang}/" /></head></html>`;
        await fs.writeFile(path.join(outputDir, 'index.html'), redirectHTML.trim());
        
        console.log("\Build complete! The root site is ready for deployment.");
